        ...
        <div class="card">
            <div class="card-header d-flex flex-wrap justify-content-between align-items-center">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('user.staff.manager.contracts') }}" class="btn btn-sm btn--dark">
                        <i class="las la-arrow-left"></i> @lang('Quay lại')
                    </a>
                    
                    <a href="{{ route('user.invest.contract.download', $invest->id) }}" class="btn btn-sm btn--success">
                        <i class="las la-download"></i> @lang('Tải PDF')
                    </a>
                    
                    @if($invest->status == \App\Constants\Status::INVEST_PENDING && $invest->staff_id && in_array($invest->staff_id, auth()->user()->staffMembers->pluck('id')->toArray()))
                    <button type="button" class="btn btn-sm btn--success approveBtn" data-bs-toggle="modal" data-bs-target="#approveModal" data-id="{{ $invest->id }}">
                        <i class="las la-check-circle"></i> @lang('Duyệt hợp đồng')
                    </button>
                    <button type="button" class="btn btn-sm btn--danger rejectBtn" data-bs-toggle="modal" data-bs-target="#rejectModal" data-id="{{ $invest->id }}">
                        <i class="las la-times-circle"></i> @lang('Từ chối')
                    </button>
                    @endif
                </div>
            </div>
                            ...
                            <div class="card-body">
                                <ul class="list-group list-group-flush">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between px-0">
                                        ...
                                        <span class="fw-bold">{{ $invest->getDisplayRoiPercentageAttribute() }}%</span>
                                    </li>
                                    <li class="list-group-item d-flex justify-content-between px-0">
                                        <span>@lang('Tiền lãi mỗi kỳ')</span>
                                        <span class="fw-bold">{{ showAmount(($invest->total_price * ($invest->getDisplayRoiPercentageAttribute()/100)) / 12) }}</span>
                                    </li>
                                    <li class="list-group-item d-flex justify-content-between px-0">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between px-0">
                                        ...
                                        <span class="fw-bold">{{ $invest->referrer->fullname ?? $invest->referral_code }}</span>
                                    </li>
                                    @endif
                                </ul>
                            </div>
                            ...
                            <div class="card-body">
                                <div class="row mb-3">
                                    @php
                                        ...
                                        $roiPercentage = $invest->getDisplayRoiPercentageAttribute() > 0 ? $invest->getDisplayRoiPercentageAttribute() : 0;
                                        ...
                                        $totalEarnings = $annualROI * ($repeatTimes / 12) * $quantity;
                                    ...
                                    <div class="col-md-6">
                                        <div class="list-group list-group-flush">
                                            <div class="list-group-item d-flex justify-content-between px-0">
                                                <span>@lang('Lãi suất hàng năm')</span>
                                                <span class="fw-bold text-primary">{{ $roiPercentage }}%</span>
                                            </div>
                                            <div class="list-group-item d-flex justify-content-between px-0">
                                                <span>@lang('Tiền lãi mỗi tháng')</span>
                                                <span class="fw-bold text-primary">{{ showAmount($monthlyPayAmount) }}</span>
                                            </div>
                                            <div class="list-group-item d-flex justify-content-between px-0">
                                            ...
                                            <div class="list-group-item d-flex justify-content-between px-0">
                                                ...
                                                <span class="fw-bold">{{ $invest->period > 0 ? $invest->period : $projectDuration }} @lang('kỳ')</span>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">@lang('Xác nhận duyệt hợp đồng')</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('user.staff.manager.approve_contract', $invest->id) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <p>@lang('Bạn có chắc chắn muốn duyệt hợp đồng này không?')</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn--dark" data-bs-dismiss="modal">@lang('Đóng')</button>
                    <button type="submit" class="btn btn--success">@lang('Xác nhận')</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="rejectModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">@lang('Xác nhận từ chối hợp đồng')</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('user.staff.manager.reject_contract', $invest->id) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label for="rejection_reason">@lang('Lý do từ chối')</label>
